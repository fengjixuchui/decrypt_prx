...
/* Eecode a bittree starting from MSB */
...
/* ring buffer: 65024+256+256: window + content_buffer + cmp_buffer */
...
int lzrc_compress(void *out, int out_len, void *in, int in_len)
{
	...
	while (1) {
...
#if 0
		re_find_match(&re, &match_len, &match_dist);
#else
		fill_buffer(&re);
		...
		if(match_len==1 || (match_len<4 && match_dist>255)){
			...
			/* encode number of bits of match distance */
			dist_state = 0;
			limit = 8;
			if(match_len>3){
				dist_state += 7;
